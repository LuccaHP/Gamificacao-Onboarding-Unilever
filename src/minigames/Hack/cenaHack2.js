...
export default class cenaHack2 extends Phaser.Scene {
    constructor(){
        super({key: 'cenaHack2'});
    }

    init(data) {
        this.hud = data.hud; //recebe informações atualizadas da HUD
    ...
    create ()
    {
        ...
        this.add.image(400, 300, 'fundoHack2');


        // Cria algumas 'áreas de destino' para os blocos
        ...
        const block1 = this.add.sprite(100, 192, 'sigla6', 1).setScale(0.18);
        const block2 = this.add.sprite(100, 448, 'sigla5', 1).setScale(0.18);
        const block3 = this.add.sprite(100, 320, 'sigla4', 1).setScale(0.18);
        ...
        this.input.on('dragend', (pointer, gameObject) => {
            //Coleta posicoes do bloco arrastado
            ...
            else if (block2.x === 512 && block2.y === 320 && !over2)
            {
                over2 = true;
                gameObject.setFrame(0);
                gameObject.disableInteractive();
            ...
            else if (block3.x === 512 && block3.y === 192 && !over3)
            {
                over3 = true;
                gameObject.setFrame(0);
                gameObject.disableInteractive();
            ...
            if(block2.x === 512 && block2.y === 448 || block2.x === 512 && block2.y === 192){
                this.scene.start('cenaGameOver', this.game);
            }
            if(block3.x === 512 && block3.y === 448|| block3.x === 512 && block3.y === 320){
                this.scene.start('cenaGameOver', this.game);
            }
            if(over1 == true && over2 == true && over3 == true){
            this.time.delayedCall(1000, () => {
                this.scene.start('cenaMapaHack', {hud: this.hud, nomeJogador: this.nomeJogador});//Volta para cenaEscritorio definindo nova posicao do player
                if (this.scene.get('cenaHack1').musicaHack) {
                this.scene.get('cenaHack1').musicaHack.stop();
